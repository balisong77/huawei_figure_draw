...
targets = ["32-all", "64-all", "96-all", "128-all", "256-all", "32-small", "64-small", "96-small", "128-small", "256-small"]
labels = ["32-all", "64-all", "96-all", "128-all", "256-all", "32-small", "64-small", "96-small", "128-small", "256-small"]


## data 
# 读取 CSV 文件
df_all = pd.read_csv(os.path.join(DATA_PATH, 'settings_big_node4.csv'))
df_all['tag'] = df_all['tag'].apply(add_suffix_all)
df_small = pd.read_csv(os.path.join(DATA_PATH, 'settings_small_node4.csv'))
df_small['tag'] = df_small['tag'].apply(add_suffix_small)
    
df = pd.concat([df_all, df_small], axis=0)
