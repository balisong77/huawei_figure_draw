...
targets =  [32, 48, 64, 96, 128, 192, 256]
x_tick_labels = [str(b) for b in targets]
## data 
# 读取 CSV 文件

df_base = pd.read_csv(os.path.join(DATA_PATH, 'settings_all_detail_node3.csv'))
df_base["group"] = "real"
df_base= df_base[df_base['tag'].isin(targets)]
df_simulator = pd.read_csv(os.path.join(DATA_PATH, 'simulator_base.csv'))
df_simulator["group"] = "simulator"
df_simulator = df_simulator[df_simulator['tag'].isin(targets)]
# df = pd.concat([df_base, df_simulator], axis=0)
# df['throughput_mpps'] = df['avg_throughput_pkts'] / (1000 * 1000)
# filtered_df = df[df['tag'].isin(targets)]

real_throughput = df_base['avg_throughput_pkts'] / (1000 * 1000)
sim_throughput = df_simulator['avg_throughput_pkts'] / (1000 * 1000)

fig_config = {
    'xlabel' : 'Batchsize',   #x轴标签名
    'ylabel_0' : 'Throughput (Mpps)' , #y轴标签名
...
with plt.style.context(styles):   
    ...
    __barcontainer = ax1.bar(x - width/2, np.array(real_throughput), width, **real_bar_config["matplot_config"])
    set_bar_edgecolor_to_black(__barcontainer)
    set_hatch(**ga_bar_config["hatch_config"])
    __barcontainer = ax1.bar(x + width/2, np.array(sim_throughput), width, **sim_bar_config["matplot_config"])
    set_bar_edgecolor_to_black(__barcontainer)
    ...
    ax1.set_xticks(x_tick, x_tick_labels, fontsize=25)
    ax1.set_ylim(0,5)
